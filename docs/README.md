...
- 実機モードは PWM0（BCM GPIO18、物理ピン 12）と `/dev/video0` を固定で使用する．連続回転サーボの給餌方向は中立位置の 1,500 µs より小さい側とし、給餌速度 1% を 1,492 µs、100% を 700 µs として線形変換する．20 ms 周期の給餌パルスを設定してから PWM を有効化し、設定した回転時間後は停止パルスを送らず PWM を無効化する．
