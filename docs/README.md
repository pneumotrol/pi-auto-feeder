...
- 実機モードは PWM0（BCM GPIO18、物理ピン 12）と `/dev/video0` を固定で使用する．連続回転サーボの給餌速度 1～100% を 1,508～2,300 µs へ線形変換し、20 ms 周期の給餌パルスを設定してから PWM を有効化する．設定した回転時間後は停止パルスを送らず PWM を無効化する．
