...
- 実機では PWM0（BCM GPIO18、物理ピン 12）を 20 ms 周期で使用する．連続回転サーボの制御パルス幅は 700～2,300 µs とする．給餌速度 1～100% を 1,508～2,300 µs へ線形変換し、給餌パルスを設定してから PWM を有効化する．設定した回転時間後は停止パルスを送らず PWM を無効化する．
